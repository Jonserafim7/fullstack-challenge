...
export interface MessageEnvelope<TPayload = unknown> {
  // The producer-assigned idempotency key (e.g. `smoke:{id}`, later `debit:{betId}`). A redelivery
  // carries the same key, letting the consumer's inbox treat it as a no-op.
  messageKey: string;
