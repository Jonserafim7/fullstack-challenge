...
- **Why the per-message TTL does not leak onto the inbox:** RabbitMQ removes a message's `expiration` property when it dead-letters the message because that TTL expired (the original value is preserved only in the `x-death` `original-expiration` field). So the message lands back on the inbox with no live TTL and cannot expire off the inbox to the DLQ — the delay applies exactly once, on the retry queue.
