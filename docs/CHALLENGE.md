...
> **Enunciado original do desafio (Jungle Gaming).** Este arquivo é o _briefing_ recebido,
> preservado **verbatim** como a fonte da verdade dos requisitos: regras do jogo, contrato da API,
> critérios de avaliação e bônus. Não edite — alterações de produto pertencem ao código e às
> decisões de implementação ficam nos [ADRs](./adr/). A visão do que **foi entregue** (setup,
> arquitetura, trade-offs) está no [`README.md`](../README.md) da raiz.
