...
- Run workspace scripts with `bun run --filter='@crash/frontend' <script>`.
- After full scaffolding, `frontend/Dockerfile` + the `frontend` block in `docker-compose.yml` serve it on :3000.
