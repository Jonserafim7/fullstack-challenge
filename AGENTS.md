...
- Run frontend scripts from the `frontend/` directory (`bun run <script>`) — it is standalone, not a root workspace. It has its own React-aware ESLint config; `bun run check` = lint + typecheck. `lefthook` runs this automatically when frontend `.ts/.tsx` files are staged (the root lint/typecheck cover only services + packages).
