...
import {
  createHashChain,
  verifyChainLink,
} from "../../src/domain/provably-fair/hash-chain";
import { deriveCrashPointHundredths } from "../../src/domain/provably-fair/crash-point";

// A fixed terminal seed makes the whole chain deterministic, so these vectors are pinned:
// recomputing them from the seeds below must always reproduce the same values.
